...
import java.awt.event.*;
  ...
  public class reportA extends JFrame
   {
	...
	getContentPane().add(lpic);
		...
		getContentPane().add(wname);
                ...
                wname1 = new JLabel("Mr. BIMAL P");	
                wname1.setFont(new Font("Serif",Font.BOLD,20));
                wname1.setBounds(275,380,150,30);
		...
		getContentPane().add(wcontact1);
                ...
                wcontact = new JLabel("8891000580");
	...
	public void countRoom()
	{
        try
        {
        rs=statement.executeQuery("select totalSeat,vacentSeat from hostelStatus where hostelName='A'");
        rs.next();
	...
	try
	 ...
	 try {
         	 ...
         	 connection= DriverManager.getConnection("jdbc:mysql://localhost:3306/root","root","jaguar");  
