...
import java.awt.event.*;
  ...
  public class reportC extends JFrame
   {
	...
	Icon icon=new ImageIcon("photo/cc.jpg");
        JLabel lpic=new JLabel(icon);
        lpic.setBounds(0,-100,500,400);
	getContentPane().add(lpic);
	
               lhead = new JLabel("Hostel C");
		...
		getContentPane().add(wcontact1);
                ...
                wcontact = new JLabel("9446647271");
	...
	public void countRoom()
	{
        try
        {
        rs=statement.executeQuery("select totalSeat,vacentSeat from hostelStatus where hostelName='C'");
        rs.next();
